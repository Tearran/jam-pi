...
#
# @stdout A The menus for catagory: $catagory
...
get_choice() {
    ...
    if [[ $DISPLAY ]] && command -v zenity >/dev/null 2>&1; then
        ...
        choice=$(whiptail --title "Functions" --menu "Choose a function:" 0 0 9 "${options[@]}" 3>&1 1>&2 2>&3)
    elif command -v dialog >/dev/null 2>&1; then
        choice=$(dialog --title "Functions" --menu "Choose a function:" 0 0 9 "${options[@]}" 3>&1 1>&2 2>&3)
    else
